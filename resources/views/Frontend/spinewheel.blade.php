...
<main class="mt-3">
    ...
    <div class="wheel">
        ...
        @foreach ($wheelslice as $slice)
            <div class="slice" style="--i: {{ $loop->iteration }}">
                <p class="value text">
                    {{ $slice->slice_one }} <!-- Access slice_one field -->
                </p>
            </div>
            <div class="slice" style="--i: 2">
                <p class="value">{{ $slice->slice_two }} </p>
            </div>
            <div class="slice" style="--i: 3">
                <p class="value">{{ $slice->slice_three }}</p>
            </div>
            <div class="slice" style="--i: 4">
                <p class="value">{{ $slice->slice_four }}</p>
            </div>
            <div class="slice" style="--i: 5">
                <p class="value">{{ $slice->slice_five }}</p>
            </div>
            <div class="slice" style="--i: 6">
                <p class="value">{{ $slice->slice_six }}</p>
            </div>
            <div class="slice" style="--i: 7">
                <p class="value">{{ $slice->slice_seven }}</p>
            </div>
            <div class="slice" style="--i: 8">
                <p class="value">{{ $slice->slice_eight }}</p>
            </div>
        @endforeach
    </div>
</main>
    ...
    <div id="myModal1" class="modal">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h2>{{ Str::ucfirst($details->title) }}</h2>
            </div>
            <p id="text">{!! $details->description !!}</p>
        </div>
    </div>
</section>
